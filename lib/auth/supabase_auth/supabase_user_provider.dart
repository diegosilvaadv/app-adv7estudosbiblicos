...
class ADVEstudosBiblicosSupabaseUser extends BaseAuthUser {
  ADVEstudosBiblicosSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> aDVEstudosBiblicosSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ADVEstudosBiblicosSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
